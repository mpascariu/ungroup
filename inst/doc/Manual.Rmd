...
All functions are documented in the standard way, which means that once you load the package using `library(pclm)` you can just type for example `?pclm` to see the help file.
...
If we want to optimize only the degree of the splines needed to create equally-spaced B-splines basis we can set `deg = NA` and assign a fixed value to the other smoothing parameters (`lambda` and `kr`).
