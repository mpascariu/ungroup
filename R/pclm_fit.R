...
#' @keywords internal
compute_standard_errors <- function(X) {
  with(X, {
...
#' 
#' @inheritParams compute_standard_errors
#' @inheritParams pclm
#' @keywords internal
#' 
pclm.confidence <- function(X, ci.level, pclm.type) {
  SE <- compute_standard_errors(X)
    
  with(X, {
    qn    <- qnorm(1 - ci.level/2)
    lower <- as.numeric(exp(eta - qn*SE))
    upper <- as.numeric(exp(eta + qn*SE))
    ...
    #output
    out <- list(fit = fit, lower = lower, upper = upper, s.e. = SE)
    return(out)
  })
}
